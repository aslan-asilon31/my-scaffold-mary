<div :title="$title" shadow separator class="border shadow">
  ...
  <x-form wire:submit="{{ $id ? 'update' : 'store' }}" wire:confirm="Are you sure?">
    ...
    <x-choices-offline wire:model="masterForm.product_category_second_id" label="Product Category Second"
      :options="$options['product_category_seconds']" placeholder="Search ..." single searchable :readonly="$isReadonly" />
    ...
    <x-file wire:model="masterForm.header_image_url" label="Header Image" accept="image/*" crop-after-change
      :disabled="$isDisabled">
      <img
        src="{{ $masterForm->header_image_url ?? 'https://upload.wikimedia.org/wikipedia/commons/1/14/No_Image_Available.jpg?20200913095930' }}"
        class="h-48 rounded-lg" />
    </x-file>
