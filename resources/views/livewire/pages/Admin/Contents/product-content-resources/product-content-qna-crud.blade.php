<x-card :title="$title" shadow separator class="border shadow">
  ...
  <x-modal wire:model="crudModal" persistent>
    ...
    <x-form wire:submit="{{ $masterId ? 'update' : 'store' }}" wire:confirm="Are you sure?">
      ...
      <x-input wire:model="masterForm.question" label="Question" placeholder="Question" :readonly="$isReadonly" />

      <x-input wire:model="masterForm.answer" label="Answer" placeholder="Answer" :readonly="$isReadonly" />
