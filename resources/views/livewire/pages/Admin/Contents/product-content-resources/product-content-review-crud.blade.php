<x-card :title="$title" shadow separator class="border shadow">
  ...
  <x-modal wire:model="crudModal" persistent>
    ...
    <x-form wire:submit="{{ $masterId ? 'update' : 'store' }}" wire:confirm="Are you sure?">
      ...
      <x-input type="number" wire:model="masterForm.rating" label="Rating" placeholder="Rating" step="any"
        :readonly="$isReadonly" />

      <x-input wire:model="masterForm.comment" label="Comment" placeholder="Comment" :readonly="$isReadonly" />
