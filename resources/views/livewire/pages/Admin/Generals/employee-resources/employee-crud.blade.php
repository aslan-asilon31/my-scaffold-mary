<x-card :title="$title" shadow separator class="border shadow">
  ...
  <x-form wire:submit="{{ $id ? 'update' : 'store' }}" wire:confirm="Are you sure?">
    ...
    <x-choices-offline wire:model="masterForm.position_id" label="Position"
      :options="$options['positions']" placeholder="Search ..." single searchable :readonly="$isReadonly" />

    <x-input wire:model="masterForm.name" label="Name" placeholder="Name" :readonly="$isReadonly" />

    <x-input wire:model="masterForm.phone" label="Phone" placeholder="Phone" :readonly="$isReadonly" />

    <x-input wire:model="masterForm.email" label="Email" placeholder="Email" :readonly="$isReadonly" />
