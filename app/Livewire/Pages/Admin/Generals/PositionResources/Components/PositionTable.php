...
namespace App\Livewire\Pages\Admin\Generals\PositionResources\Components;
...
{
    public string $tableName = 'position-table-hs6ulb-table';
    ...
    public function fields(): PowerGridFields
    {
        ...
        ->add('action', fn($record) => Blade::render('
            <x-dropdown no-x-anchor class="btn-sm">
                <x-menu-item title="Show" Link="/positions/show/' . e($record->id) . '/readonly" />
                <x-menu-item title="Edit" Link="/positions/edit/' . e($record->id) . '"/>
            </x-dropdown>'))
        ->add('id')
        ->add('name')
